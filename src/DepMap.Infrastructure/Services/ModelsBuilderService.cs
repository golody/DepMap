...
public class ModelsBuilderService : IModelsBuilderService
{
    public (List<NodeModel> nodes, List<LinkModel> links) 
        GenerateNodesAndLinks(List<Service> services, List<ControllerDescription> controllers, List<Middleware> middleware)
    {
    ...
    public List<LinkModel> GenerateLinks(List<Service> services)
    {
        List<LinkModel> links = [];
        foreach (Service service in services)
        { }
    ...
    {
        if (service.AbstractionType.Namespace != null && service.AbstractionType.Namespace.Contains("Microsoft"))
        {
            return;
        }

        // Create node without group if service has no interface or abstract class
